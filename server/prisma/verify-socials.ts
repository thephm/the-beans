...
async function verify() {
  ...
  try {
    ...
    // Check if old columns still exist by querying information_schema
    const columnCheck = await prisma.$queryRawUnsafe<[{ exists: boolean }]>(
      `SELECT EXISTS (
         SELECT 1 FROM information_schema.columns 
         WHERE table_name = 'roasters' AND column_name = 'instagram'
       ) as exists`
    );

    const oldColumnsExist = columnCheck[0]?.exists;

    if (!oldColumnsExist) {
      console.log('✓ Migration already completed - old social media columns have been removed.');
      console.log('✓ Verification passed - socialNetworks field is in use.');
      await prisma.$disconnect();
      process.exit(0);
    }

    // If old columns still exist, check for unmigrated data
