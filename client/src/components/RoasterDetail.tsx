...
const RoasterDetail: React.FC<{ id: string }> = ({ id }) => {
  ...
  const fetchRoaster = async (roasterId: string) => {
    try {
      setLoading(true);
      const roasterData = await apiClient.getRoaster(roasterId) as Roaster;
      ...
      setIsFavourited(roasterData.isFavourited || false);
    } catch (err) {
  ...
  const toggleFavourited = async () => {
    ...
    try {
      if (isFavourited) {
        await apiClient.removeFavourite(roaster.id);
      } else {
        await apiClient.addFavourite(roaster.id);
      }
